...
function footerText(context) {
  return `created by @unstoppable_neid | LimeHub ${context} | Today ${new Date().toLocaleString(
    "id-ID"
  )}`;
}
...
const client = new Client({
  intents: [
    ...
    GatewayIntentBits.GuildVoiceStates, // buat pantau Stage
  ],
});
...
const HELPER_IDS = (process.env.HELPER_IDS || "")
  .split(",")
  .map((x) => x.trim())
  .filter(Boolean); // id helper yang dipantau absen

const SPECIAL_HELPER_ID = process.env.SPECIAL_HELPER_ID || null; // owner id

// =====================================
// OWNER CONFIG (PASSWORD) + JSON
// =====================================
...
// user yg sudah pernah klik owner
let usedOwnerSelector = new Set();
...
try {
  ...
  console.error("❌ Gagal load owner_selector.json:", err);
}
...
try {
  ...
  console.error("❌ Gagal load owner_config.json:", err);
}

function saveUsedOwnerSelector() {
  try {
    const arr = [...usedOwnerSelector];
    fs.writeFileSync(OWNER_STORE_PATH, JSON.stringify(arr, null, 2), "utf8");
  } catch (err) {
    console.error("❌ Gagal save owner_selector.json:", err);
  }
}
...
function saveOwnerConfig() {
  try {
    ...
    console.error("❌ Gagal save owner_config.json:", err);
  }
}
...
// ABSEN STATE
// =====================================
// staffData[id] = { status, msgCount, msgPoints, dutyMs, dutyPoints }
const staffData = new Map();
const blacklistedChannels = new Set();
const stageSessions = new Map(); // helperId -> { channelId, startedAt }

const FIVE_HOURS_MS = 5 * 60 * 60 * 1000;

// inisialisasi helper di absen
...
// helper yg status ON -> dipakai selector helper
function getOnlineHelpersForUser(requesterId) {
  const result = [];
  for (const id of HELPER_IDS) {
    const data = staffData.get(id);
    if (!data) continue;
    if (data.status === "on") {
      if (SPECIAL_HELPER_ID && usedOwnerSelector.has(requesterId)) {
        // user ini sudah pernah pakai owner → jangan tampilkan owner lagi
        if (id === SPECIAL_HELPER_ID) continue;
      }
      result.push(id);
    }
  }
...
// TICKET STATE + FAQ
// =====================================
const claimedTickets = new Set(); // buat queue
let faqIdCounter = 0;
...
const commands = [
  ...
  new SlashCommandBuilder()
    .setName("owner")
    .setDescription("Pengaturan owner helper.")
    .addSubcommand((sub) =>
      sub
        .setName("setpassword")
        .setDescription("Set password owner untuk selector.")
        .addStringOption((opt) =>
          opt
            .setName("password")
  ...
  new SlashCommandBuilder()
    .setName("ownerreset")
    .setDescription("Reset daftar user yang pernah call owner.")
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .setDMPermission(false),
].map((cmd) => cmd.toJSON());
  ...
  if (channel.name.startsWith("ticket-")) {
    if (message.content.trim().toLowerCase() === ".done") {
      ...
      const doneEmbed = new EmbedBuilder()
        ...
        .setImage(TUTORIAL_IMAGE_URL || FOOTER_GIF)
...
client.on(Events.VoiceStateUpdate, (oldState, newState) => {
  ...
  // helper join stage dan status ON → mulai sesi
  if (!wasStage && isStage) {
    if (data.status === "on") {
  ...
  // helper keluar dari stage → akumulasi waktu
  if (wasStage && !isStage) {
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  if (interaction.isChatInputCommand()) {
    ...
    if (commandName === "absen") {
      ...
      for (const id of HELPER_IDS) {
        ...
        const totalPoints = data.msgPoints + data.dutyPoints;
        ...
        lines.push(
          [
            `**${memberName}** • Status: \`${data.status.toUpperCase()}\``,
    ...
    if (commandName === "blacklist") {
      ...
      if (blacklistedChannels.has(channel.id)) {
        blacklistedChannels.delete(channel.id);
        return interaction.reply({
          content: `✅ Channel ${channel} **dihapus** dari blacklist.`,
          ephemeral: true,
        });
      } else {
        blacklistedChannels.add(channel.id);
        return interaction.reply({
          content: `✅ Channel ${channel} **ditambahkan** ke blacklist.`,
          ephemeral: true,
        });
      }
    }
    ...
    if (commandName === "owner") {
      ...
      if (sub === "setpassword") {
        ...
        await interaction.reply({
          content:
            "✅ Password owner untuk selector sudah di-set. (Password tidak ditampilkan.)",
          ephemeral: true,
        });
      }
    }
    ...
    if (commandName === "ownerreset") {
      ...
      try {
        ...
        console.error("❌ Gagal hapus owner_selector.json:", err);
      }
      await interaction.reply({
        content: "✅ Daftar user yang pernah call owner sudah di-reset.",
        ephemeral: true,
      });
    }
  ...
  if (interaction.isButton()) {
    ...
    if (
      ...
      const data = staffData.get(userId);
      if (!data) {
        return interaction.reply({
      ...
      // handle stage session kalau status berubah
      const guild = interaction.guild;
      const member = await guild.members.fetch(userId).catch(() => null);
      if (member) {
        const ch = member.voice.channel;
        const isStage = ch && ch.type === ChannelType.GuildStageVoice;
        const session = stageSessions.get(userId);

        // kalau sebelumnya ON dan sekarang bukan ON → tutup sesi
        if (oldStatus === "on" && newStatus !== "on" && session) {
          const delta = Date.now() - session.startedAt;
          data.dutyMs += delta;
          stageSessions.delete(userId);
          while (data.dutyMs >= FIVE_HOURS_MS) {
            data.dutyMs -= FIVE_HOURS_MS;
            data.dutyPoints += 1;
          }
        ...
        // kalau sebelumnya bukan ON, sekarang ON dan lagi di stage → mulai sesi baru
        if (oldStatus !== "on" && newStatus === "on" && isStage) {
          stageSessions.set(userId, {
            channelId: ch.id,
            startedAt: Date.now(),
          });
        }
    ...
    if (interaction.customId === "create_ticket") {
      ...
      if (!isSupportTicket) {
        ...
        const faqOptions = faqItems.map((item) => ({
          label: item.question.slice(0, 100),
          value: item.id,
        }));
        faqOptions.push({
        ...
        const onlineHelpers = getOnlineHelpersForUser(member.id);
        const helperOptions = [];

        for (const helperId of onlineHelpers) {
          const helperMember = await guild.members
            .fetch(helperId)
            .catch(() => null);
          const label =
            helperMember?.displayName || `Helper ${helperId.slice(0, 6)}...`;
          helperOptions.push({ label, value: helperId });
        }

        if (
          SPECIAL_HELPER_ID &&
          onlineHelpers.includes(SPECIAL_HELPER_ID) &&
          !usedOwnerSelector.has(member.id)
        ) {
          helperOptions.push({
            label: "🚨 Contact Owner (priority)",
    ...
    if (interaction.customId === "support_helper_select") {
      ...
      if (value.startsWith("owner:")) {
        ...
        if (!ownerConfig.password) {
          return interaction.reply({
            content:
              "❌ Password owner belum diset. Gunakan `/owner setpassword` dulu.",
            ephemeral: true,
          });
        }
        ...
        await interaction.reply({
          content:
            "🔐 Masukkan password owner (kirim 1x pesan di channel ini, pesan akan dihapus):",
          ephemeral: true,
        });
