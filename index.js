// index.js — LimeHub All-in-One (Ticket + Absen + Bilingual + Logging)
...
function formatLogLine(level, message, extra) {
...
function logError(errOrMsg, extra) {
  let message = "";
  if (errOrMsg instanceof Error) {
    message = errOrMsg.stack || errOrMsg.message || String(errOrMsg);
  } else {
...
process.on("unhandledRejection", (reason) => {
  logError(reason, { context: "unhandledRejection" });
});
...
// =====================================
// FILE CONFIG (ABSEN STAFF + OWNER + ABSEN DATA)
// =====================================
...
const ABSEN_DATA_PATH = path.join(__dirname, "absen_data.json");
...
try {
  if (fs.existsSync(ABSEN_STAFF_PATH)) {
    ...
    if (Array.isArray(arr)) {
      helperIds = arr.map(String);
      logInfo("Loaded helper IDs from absen_staff.json", {
        count: helperIds.length,
      });
    }
  }
} catch (e) {
  logError(e, { context: "load ABSEN_STAFF_PATH" });
}
if (!helperIds.length) {
  helperIds = [...HELPER_IDS_ENV];
  logInfo("Using HELPER_IDS from env", { count: helperIds.length });
}
...
function saveHelperIds() {
  try {
    ...
    logInfo("Saved helper IDs to absen_staff.json", {
      count: helperIds.length,
    });
  } catch (e) {
    logError(e, { context: "saveHelperIds" });
  }
}
...
try {
  if (fs.existsSync(OWNER_STORE_PATH)) {
    ...
    const arr = JSON.parse(raw);
    if (Array.isArray(arr)) {
      usedOwnerSelector = new Set(arr);
      logInfo("Loaded owner selector store", { count: arr.length });
    }
  }
} catch (e) {
  logError(e, { context: "load OWNER_STORE_PATH" });
}
...
try {
  if (fs.existsSync(OWNER_CONFIG_PATH)) {
    ...
    if (obj && typeof obj === "object") {
      ownerConfig = obj;
      logInfo("Loaded owner_config.json", { hasPassword: !!obj.password });
    }
  }
} catch (e) {
  logError(e, { context: "load OWNER_CONFIG_PATH" });
}
...
function saveUsedOwnerSelector() {
  try {
    ...
    logInfo("Saved owner selector store", {
      count: usedOwnerSelector.size,
    });
  } catch (e) {
    logError(e, { context: "saveUsedOwnerSelector" });
  }
}
...
function saveOwnerConfig() {
  try {
    ...
    logInfo("Saved owner_config.json");
  } catch (e) {
    logError(e, { context: "saveOwnerConfig" });
  }
}
...
// ====== LOAD / SAVE ABSEN DATA (POINTS & STATUS) ======
function loadAbsenData() {
  try {
    if (!fs.existsSync(ABSEN_DATA_PATH)) {
      logInfo("No absen_data.json found, starting fresh");
      return;
    }

    const raw = fs.readFileSync(ABSEN_DATA_PATH, "utf8");
    const obj = JSON.parse(raw);
    ...
    for (const id of Object.keys(obj)) {
      if (!helperIds.includes(id)) continue; // helper sudah tidak terdaftar, skip

      const d = obj[id] || {};
      staffData.set(id, {
        status: d.status || "off",
        msgCount: Number(d.msgCount) || 0,
        msgPoints: Number(d.msgPoints) || 0,
        dutyMs: Number(d.dutyMs) || 0,
        dutyPoints: Number(d.dutyPoints) || 0,
        ticketPoints: Number(d.ticketPoints) || 0,
      });

      restored++;
    }

    logInfo("Absen data loaded from JSON", {
      helpers: restored,
      path: ABSEN_DATA_PATH,
    });
  } catch (e) {
    logError(e, { context: "loadAbsenData" });
  }
}
...
function saveAbsenData() {
  try {
    const out = {};
    for (const id of helperIds) {
      const d = staffData.get(id);
      if (!d) continue;

      out[id] = {
        status: d.status,
        msgCount: d.msgCount,
        msgPoints: d.msgPoints,
        dutyMs: d.dutyMs,
        dutyPoints: d.dutyPoints,
        ticketPoints: d.ticketPoints,
      };
    }

    fs.writeFileSync(ABSEN_DATA_PATH, JSON.stringify(out, null, 2), "utf8");

    logInfo("Absen data saved to JSON", {
      helpers: Object.keys(out).length,
      path: ABSEN_DATA_PATH,
    });
  } catch (e) {
    logError(e, { context: "saveAbsenData" });
  }
}

// load data ketika start
loadAbsenData();
...
const commands = [
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addStringOption((opt) =>
          opt
            .setName("question")
            .setDescription("Frequently asked question.")
            .setRequired(true)
        )
        .addStringOption((opt) =>
          opt
  ...
  // /absen panel + /absen recap + /absen staff
  new SlashCommandBuilder()
    .setName("absen")
    .setDescription("Staff attendance panel & configuration.")
    .addSubcommand((sub) =>
      sub.setName("panel").setDescription("Show staff attendance panel.")
    )
    .addSubcommand((sub) =>
      sub
        .setName("recap")
        .setDescription("Show staff attendance recap in one embed.")
    )
    .addSubcommand((sub) =>
      sub
        .setName("staff")
  ...
  new SlashCommandBuilder()
    ...
    .addChannelOption((opt) =>
      opt
        .setName("channel")
        .setDescription("Channel to be blacklisted / un-blacklisted.")
...
async function registerCommands() {
  ...
  try {
    ...
    logError(e, { context: "registerCommands" });
  }
}
...
client.once(Events.ClientReady, async (c) => {
  logInfo(`Logged in as ${c.user.tag} (LimeHub All-in-One)`, {
    userId: c.user.id,
  });
...
function buildAbsenPanel() {
  ...
  for (const id of helperIds) {
    ...
    const bar = (p) => {
      const total = 10;
      const filled = Math.round(Math.max(0, Math.min(1, p)) * total);
  ...
  const body =
    lines.join("\n") ||
    "There is no staff helper data yet.\nAdd staff via `/absen staff`.";

  const desc = ["Staff activity and attendance overview.", "", body].join("\n");
...
// =====================================
// ABSEN RECAP EMBED (1 EMBED)
// =====================================
function buildAbsenRecapEmbed() {
  const rows = [];
  ...
  for (const id of helperIds) {
    ...
    const total =
      (Number(d.msgPoints) || 0) +
      (Number(d.dutyPoints) || 0) +
      (Number(d.ticketPoints) || 0);

    rows.push({ id, data: d, total });
  }

  if (!rows.length) {
    return new EmbedBuilder()
      .setColor(THEME_COLOR)
      .setTitle("📒 LimeHub Staff Attendance Recap")
      .setDescription(
        "There is no staff attendance data yet.\nAdd staff via `/absen staff` first."
      )
      .setImage(FOOTER_GIF)
      .setFooter({ text: footerText("Absen Recap") });
  }
  ...
  for (const row of rows) {
    ...
    const minutes = Math.round((Number(d.dutyMs) || 0) / 60000);

    lines.push(
      [
        `**${index}. <@${row.id}>**`,
        `> Status: \`${(d.status || "off").toUpperCase()}\``,
        `> Chat: \`${d.msgCount || 0} msg\` → \`${d.msgPoints || 0} pts\``,
        `> Stage: \`${minutes} min\` → \`${d.dutyPoints || 0} pts\``,
        `> Tickets: \`${d.ticketPoints || 0} pts\``,
  ...
  // batasi biar gak tembus limit 4096 char
  let desc = "";
  for (const block of lines) {
    if ((desc + "\n\n" + block).length > 3800) break;
    desc = desc ? desc + "\n\n" + block : block;
  }
  ...
  try {
    ...
    if (channel.name.includes("🛒")) {
      if (message.content.trim().toLowerCase() === ".done") {
        ...
        if (!canUseDone) {
          return message.reply("❌ Only staff/helpers can use `.done`.");
        }
        ...
        const doneEmbed = new EmbedBuilder()
          ...
          .setDescription(
            [
              ...
              `• Berikutnya, ambil script kamu di ${premiumMention}.`,
              "",
              "__**Cara Mengambil Script**__",
              '• Klik **"Get Script"** untuk mendapatkan script kamu.',
              "• Salin script kamu dan hapus bagian yang tidak termasuk dalam script asli.",
              "",
              "Ikuti contoh gambar di bawah ini.",
              "",
              "Jika kamu mengalami masalah saat menjalankan script, pastikan kamu telah melakukannya sesuai contoh di bawah ini.",
            ].join("\n")
          )
          .setImage(
        ...
        logInfo("Ticket marked as done via .done", {
    ...
    const queueEmbed = new EmbedBuilder()
      ...
      .addFields({
        ...
        value: ["```yaml", `Position: #${position} of ${total}`, "```"].join(
          "\n"
        ),
      })
      .addFields({
        name: "✨",
        value: "Your ticket will be processed shortly.",
      })
      .setImage(FOOTER_GIF)
      .setFooter({ text: footerText("Purchase") });
    ...
    logInfo("Queue status sent (payment proof image detected)", {
      channelId: channel.id,
      userId: message.author.id,
      position,
      total,
...
client.on(Events.VoiceStateUpdate, (oldState, newState) => {
  try {
    ...
    if (!wasStage && isStage) {
      if (data.status === "on") {
        stageSessions.set(id, {
          channelId: newChannel.id,
          startedAt: Date.now(),
        });
        logInfo("Stage session started", {
          userId: id,
          channelId: newChannel.id,
        });
      }
    ...
    if (wasStage && !isStage) {
      if (session && session.startedAt) {
        ...
        saveAbsenData();
        logInfo("Stage session ended", {
          userId: id,
          dutyMs: data.dutyMs,
          dutyPoints: data.dutyPoints,
        });
      }
      return;
    }
    ...
    if (wasStage && isStage && oldChannel.id !== newChannel.id) {
      if (session && session.startedAt) {
        ...
        saveAbsenData();
        logInfo("Stage session moved channel", {
          userId: id,
    ...
    logError(e, { context: "VoiceStateUpdate" });
  }
});
  ...
  try {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (commandName === "faq") {
        ...
        if (sub === "list") {
          if (!faqItems.length) {
            await replyOnce(interaction, {
              content: "📚 There are no FAQs saved yet.",
              ephemeral: true,
            });
            return;
          }
        ...
        if (sub === "remove") {
          ...
          const menu = new StringSelectMenuBuilder()
            ...
            .setPlaceholder(
              "Select the FAQ question you want to delete."
            )
            .addOptions(options);
      ...
      if (commandName === "absen") {
        ...
        if (sub === "recap") {
          if (
            ...
            await replyOnce(interaction, {
              content: "❌ Only admin can view attendance recap.",
              ephemeral: true,
            });
            return;
          }

          const embed = buildAbsenRecapEmbed();
          logInfo("Absen recap requested", {
        ...
        if (sub === "staff") {
          if (
            ...
            await replyOnce(interaction, {
              content: "❌ Only admin can manage staff attendance list.",
              ephemeral: true,
            });
            return;
          }
          ...
          if (action === "add") {
            ...
            logInfo("Helper added to attendance panel", { helperId: id });
          ...
          if (action === "remove") {
            ...
            logInfo("Helper removed from attendance panel", {
              helperId: id,
            });
      ...
      if (commandName === "blacklist") {
        ...
        if (blacklistedChannels.has(ch.id)) {
          ...
          logInfo("Channel removed from absen blacklist", {
            channelId: ch.id,
          });
          await replyOnce(interaction, {
          ...
          logInfo("Channel added to absen blacklist", {
            channelId: ch.id,
          });
          await replyOnce(interaction, {
      ...
      if (commandName === "owner") {
        ...
        if (sub === "setpassword") {
          ...
          saveOwnerConfig();
          logInfo("Owner password set", { by: interaction.user.id });
          await replyOnce(interaction, {
            content:
      ...
      if (commandName === "ownerreset") {
        ...
        try {
          ...
          logError(e, { context: "delete OWNER_STORE_PATH" });
        }
        logInfo("Owner selector reset by admin", {
          by: interaction.user.id,
        });
        await replyOnce(interaction, {
          content:
            "✅ The list of users who have called the admin has been reset.",
          ephemeral: true,
        });
        return;
      }
    ...
    if (interaction.isButton()) {
      ...
      if (
        ...
        await interaction.reply({
          content: `✅ Your current attendance status: \`${newStatus.toUpperCase()}\``,
          ephemeral: true,
        });
        return;
      }
      ...
      if (interaction.customId === "create_ticket") {
        ...
        const handlerMention = handlerRoleId ? `<@&${handlerRoleId}>` : "Staff";
        ...
        if (!isSupportTicket) {
          ...
          const supportEmbed = new EmbedBuilder()
            ...
            .setDescription(
              [
                `Halo ${member}, thank you for opening a support ticket on **LimeHub**.`,
                "",
                "Before our team replies directly, please choose one of the questions below that matches your issue.",
                "",
                "Many common issues are already answered in the FAQ.",
              ].join("\n")
            )
            .setImage(FOOTER_GIF)
            .setFooter({ text: footerText("Support") });
      ...
      if (interaction.customId === "claim_ticket") {
        ...
        if (helperIds.includes(interaction.user.id)) {
          ...
          if (data) {
            data.ticketPoints = (data.ticketPoints || 0) + 0.5;
            saveAbsenData();
          }
        }
      ...
      if (interaction.customId === "close_ticket") {
        ...
        logInfo("Ticket close requested", {
          channelId: channel.id,
          by: interaction.user.id,
          isSupport,
          isPurchase,
        ...
        const buildEmbed = (sec) =>
          new EmbedBuilder()
            ...
            .setDescription(`Ticket will close in **${sec} seconds**.`)
            .setImage(FOOTER_GIF)
            .setFooter({
    ...
    if (interaction.isStringSelectMenu()) {
      ...
      if (interaction.customId === "faq_remove_select") {
        ...
        logInfo("FAQ removed", { id, question: removed.question });
        await interaction.reply({
      ...
      if (interaction.customId === "support_faq_select") {
        ...
        logInfo("FAQ answer shown in ticket", {
          faqId: faq.id,
          channelId: channel.id,
        });
      ...
      if (interaction.customId === "support_helper_select") {
        ...
        if (value === "none") {
          const noEmbed = new EmbedBuilder()
            ...
            .setDescription("There are currently no helpers online.")
            .setImage(FOOTER_GIF)
            .setFooter({ text: footerText("Support") });
        ...
        if (value.startsWith("owner:")) {
          ...
          collector.on("collect", async (msg) => {
            ...
            if (pw === ownerConfig.password) {
              ...
              logInfo("Owner called with correct password", {
                userId: interaction.user.id,
                ownerId,
          ...
          collector.on("end", async (collected, reason) => {
            if (reason === "time" && collected.size === 0) {
              const timeoutEmbed = new EmbedBuilder()
                ...
                .setDescription("No password was sent. Please try again.")
                .setImage(FOOTER_GIF)
                .setFooter({ text: footerText("Support") });
        ...
        logInfo("Helper called via selector", {
          helperId,
          by: interaction.user.id,
          channelId: interaction.channel.id,
        });
...
// LOGIN
// =====================================
logInfo("Starting bot login...");
client.login(TOKEN).catch((e) => {
  logError(e, { context: "client.login" });
});
